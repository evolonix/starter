...
export function PaginationPrevious({
  ...
  children = 'Previous',
  onClick,
}: React.PropsWithChildren<{ href?: string | null; className?: string; disabled?: boolean; onClick?: () => void }>) {
  return (
    <span className={clsx(className, 'grow basis-0')}>
      <Button
        disabled={disabled}
        {...(href === null ? (onClick ? {} : { disabled: true }) : { href })}
        plain
...
export function PaginationGap({ className, children = <>&hellip;</>, ...props }: React.ComponentPropsWithoutRef<'span'>) {
  return (
    <span
      aria-hidden="true"
      {...props}
      className={clsx(className, 'w-[2.25rem] text-center text-sm/6 font-semibold text-zinc-950 select-none dark:text-white')}
    >
      {children}
    </span>
  );
