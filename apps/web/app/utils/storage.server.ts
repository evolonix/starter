...
const LOCAL_STORAGE_PATH = `${process.env.NODE_ENV === 'development' ? './_' : '/'}data/images`;
...
async function uploadToLocalStorage(file: File | FileUpload, key: string) {
  const fileStorage = new LocalFileStorage(LOCAL_STORAGE_PATH);
...
export async function uploadProfileImage(
  ...
  return process.env.NODE_ENV === 'development'
    ? uploadToLocalStorage(file, key)
    : uploadToStorage(file, key);
}

export async function removeProfileImage(key: string) {
  const fileStorage = new LocalFileStorage(LOCAL_STORAGE_PATH);
  await fileStorage.remove(key);
}

export function getProfileImage(key: string) {
  const fileStorage = new LocalFileStorage(LOCAL_STORAGE_PATH);
