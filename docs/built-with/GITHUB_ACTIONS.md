...
# Changes to files in any web folder
web:
  - changed-files:
...
- name: web
  color: '1d76db'
  description: 'Changes to files in any web folder'
```
...
jobs:
  preview:
    runs-on: ubuntu-latest
    outputs:
      affected_build_web: ${{ steps.affected_build_web.outputs.projects }}
    steps:
      - uses: actions/checkout@v4
        with:
          filter: tree:0
      ...
      - run: npx nx affected -t build --exclude web

      - id: affected_build_web
        if: github.event_name != 'workflow_dispatch'
        run: |
          npx nx show projects --projects web --affected
          projects=$(npx nx show projects --projects web --affected | tr '\n' ' ' | xargs)
          if [ -z "$projects" ]; then
            echo "No affected projects found for web."
          fi
          echo "projects=$projects" >> $GITHUB_OUTPUT

      - if: github.event_name == 'workflow_dispatch' || steps.affected_build_web.outputs.projects != ''
  ...
  deploy-web-preview:
    ...
    needs: preview
    if: github.event_name == 'workflow_dispatch' || needs.preview.outputs.affected_build_web != ''
    steps:
      - uses: actions/checkout@v4
      ...
      - id: fly_status
        run: |
          if flyctl status --app "~[ .starter.name ]~-web-pr-${{ github.event.number }}"; then
      ...
      - if: ${{ steps.fly_status.outputs.exists == 'false' }}
        run: |
          flyctl apps create --org ~[ .starter.name ]~ "~[ .starter.name ]~-web-pr-${{ github.event.number }}" \
            --name "~[ .starter.name ]~-web-pr-${{ github.event.number }}" \
            --yes
      ...
      - id: deploy_status
        run: |
          flyctl status --app "~[ .starter.name ]~-web-pr-${{ github.event.number }}" --json > status.json
...
jobs:
  cleanup-web-preview:
    ...
    steps:
      ...
      - run: |
          flyctl apps destroy "~[ .starter.name ]~-web-pr-${{ github.event.number }}" --yes
        env:
          FLY_API_TOKEN: ${{ secrets.FLY_API_TOKEN }}
...
jobs:
  ...
  deploy-web-staging:
    environment:
      name: Staging
      url: https://~[ .starter.name ]~-web-staging.fly.dev
    runs-on: ubuntu-latest
    concurrency: deploy-web-staging
    needs: staging
    if: github.event_name == 'workflow_dispatch' || needs.staging.outputs.affected_build_web != ''
    steps:
      - uses: actions/checkout@v4
...
jobs:
  deploy-web:
    environment:
      name: Production
      url: https://~[ .starter.name ]~-web.fly.dev
    runs-on: ubuntu-latest
