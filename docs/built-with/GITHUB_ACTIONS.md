...
jobs:
  ...
  deploy-web-preview:
    ...
    steps:
      ...
      - id: fly_status
        run: |
          if flyctl status --app "starter-web-pr-${{ github.event.number }}"; then
      ...
      - if: ${{ steps.fly_status.outputs.exists == 'false' }}
        run: |
          flyctl apps create --org starter "starter-web-pr-${{ github.event.number }}" \
            --name "starter-web-pr-${{ github.event.number }}" \
            --yes
      ...
      - id: deploy_status
        run: |
          flyctl status --app "starter-web-pr-${{ github.event.number }}" --json > status.json
...
jobs:
  cleanup-web-preview:
    ...
    steps:
      ...
      - run: |
          flyctl apps destroy "starter-web-pr-${{ github.event.number }}" --yes
        env:
          FLY_API_TOKEN: ${{ secrets.FLY_API_TOKEN }}
...
jobs:
  ...
  deploy-web-staging:
    environment:
      name: Staging
      url: https://starter-web-staging.fly.dev
    runs-on: ubuntu-latest
